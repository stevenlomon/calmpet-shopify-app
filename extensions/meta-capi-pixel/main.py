...
async def _process_single_event(payload: ClientPayload, request: Request, test_event_code: Optional[str] = None):
    ...
    utm_parts = [f"{k}={final_custom_data[k]}" for k in utm_keys if final_custom_data.get(k)]
    utm_str = f" | UTMs: {', '.join(utm_parts)}" if utm_parts else ""

    fbc_str = f"fbc={meta_user_data.get('fbc', 'none')}"
    fbp_str = f"fbp={'yes' if meta_user_data.get('fbp') else 'none'}"
    cookies_str = f" | {fbc_str}, {fbp_str}"

    if test_event_code:
        final_payload["test_event_code"] = test_event_code
        logging.info(f"🚀 SENDING TEST EVENT: {payload.event_name}{val_str}{cookies_str}{utm_str} | ID: {payload.event_id} | Code: {test_event_code}")
    else:
        logging.info(f"🚀 SENDING LIVE CAPI EVENT: {payload.event_name}{val_str}{cookies_str}{utm_str} | ID: {payload.event_id}")
